...
class _BackMoneyOrderState extends State<BackMoneyOrder> with AutomaticKeepAliveClientMixin{
  ...
  void getListData(int index){
    if(index == 1){
      ...
      if (model != null){
        OrderRequest.requestRefundOrderList((model as UserModel).token, 1).then((value){
          _dataList = [];
          LogUtil.d(value);
      ...
      final model = StorageUtil().getUserModel();
      if (model != null){
        OrderRequest.requestRefundOrderList((model as UserModel).token, index).then((value){
          LogUtil.d(value);
